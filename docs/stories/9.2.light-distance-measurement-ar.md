...
1. Users can launch AR measurement tool from Plant Detail → Light tab
...
- [ ] Build tutorial overlay (AC: 11)
  ...
  - [ ] "Tap light source" → "Tap canopy" → "Done"
  - [ ] Skip option
  - [ ] Persist tutorial completion in UserDefaults
- [ ] Handle edge cases and errors (AC: 10, 12)
  - [ ] ARKit unavailable (old device) → manual entry
  - [ ] Poor tracking quality → show warning
  - [ ] Low light (no LiDAR) → show flashlight suggestion
   ...
   struct LightRecommendation {
       ...
       static func evaluate(
           ...
           if distance < optimal.lowerBound {
               ...
               return (.optimal, "Perfect distance! 🎯")
           }
       }
...
**Fallback Strategy:**
- ARKit unavailable → manual entry UI
- No LiDAR + low light → suggest turning on flashlight
- Poor tracking → show recalibration guide
...
### Tasks Completed
- [x] Extended LightType enum with new cases (mh, cfl, cmh)
- [x] Extended PlantStage enum with drying and curing stages
- [x] Added lightDistance and lightType fields to Plant model
- [x] Created LightDistanceRecommendationEngine service
- [x] Created LightDistanceARView with ARKit integration
- [x] Created LightDistanceTutorial overlay
- [x] Created ManualDistanceEntryView fallback
- [x] Implemented distance calculation with AR markers
- [x] Implemented real-time distance recommendations
- [x] Added tutorial system with UserDefaults persistence
- [x] Created comprehensive unit tests for LightDistanceRecommendationEngine

### File List
**Domain Models:**
- `GroBroPackage/Sources/GroBroDomain/Models/LightType.swift` (modified)
- `GroBroPackage/Sources/GroBroDomain/Models/PlantStage.swift` (modified)
- `GroBroPackage/Sources/GroBroDomain/Models/Plant.swift` (modified)

**Domain Services:**
- `GroBroPackage/Sources/GroBroDomain/Services/LightDistanceRecommendationEngine.swift` (new)

**Feature Views:**
- `GroBroPackage/Sources/GroBroFeature/PlantDetail/Light/LightDistanceARView.swift` (new)
- `GroBroPackage/Sources/GroBroFeature/PlantDetail/Light/LightDistanceTutorial.swift` (new)
- `GroBroPackage/Sources/GroBroFeature/PlantDetail/Light/ManualDistanceEntryView.swift` (new)

**Tests:**
- `GroBroPackage/Tests/GroBroDomainTests/LightDistanceRecommendationEngineTests.swift` (new)

**Supporting Files (updated for new enum cases):**
- `GroBroPackage/Sources/GroBroDomain/Services/LightMeterService.swift` (modified)
- `GroBroPackage/Sources/GroBroDomain/Services/NutrientCalculator.swift` (modified)
- `GroBroPackage/Sources/GroBroDomain/Analytics/EnvironmentalAnalyzer.swift` (modified)
- `GroBroPackage/Sources/GroBroDomain/Models/EnvironmentalStatus.swift` (modified)

### Completion Notes
- Implemented full ARKit-based light distance measurement system
- Created beautiful glassmorphic UI following Smart Greenhouse design system
- Recommendation engine provides optimal distance ranges for all light types and stages
- Tutorial overlay guides first-time users through AR measurement process
- Manual entry fallback ensures all users can measure distance (AR not required)
- Comprehensive test coverage for recommendation logic
- iOS-only features properly wrapped in `#if os(iOS)` conditionals

### Known Limitations
- Pre-existing build issues in codebase prevent full test suite execution
- AR views require iOS 14+ and ARKit support
- Manual entry is the fallback for devices without ARKit
